...
def nb_train_smoothing(X, Y, vocab_size, num_classes, alpha):
    ...
    for c_index in range(num_classes):
        ...
        class_prob[c_index] = Y.count(c_index)/len(Y)

    alpha_array = np.ones((num_classes,vocab_size))*alpha

    class_word_prob += alpha_array

    for doc_index in range(len(X)):

        document = X[doc_index]

        c_label = Y[doc_index]

        for word in document:

            class_word_prob[c_label,word[0]] += word[1]

    for c_index in range(num_classes):

        class_word_prob[c_index,:] = class_word_prob[c_index,:]/np.sum(class_word_prob[c_index,:])
    ###################################################
...
def nb_predict(X, class_prob, class_word_prob):
    ...
    for doc_index in range(len(X)):
        ...
        for word in document:
            ...
            ham_prob = class_word_prob[0,word[0]]

            spam_prob = class_word_prob[1,word[0]]
            ...
            if ham_prob !=0 or spam_prob !=0:
                ...
                log_class_prob[0] += log_ham_prob*word[1]
                log_class_prob[1] += log_spam_prob*word[1]


        if log_class_prob[0] - log_class_prob[1] >0:
            Ypred.append(0)
        else:
            Ypred.append(1)
