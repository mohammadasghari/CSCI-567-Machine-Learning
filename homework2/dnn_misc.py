...
class linear_layer:
    ...
    def forward(self, X):
        ...
        b1_repeated = np.tile(self.params['b'],(X.shape[0],1))

        forward_output = np.dot(X,self.params['W']) + b1_repeated
    ...
    def backward(self, X, grad):
        ...
        self.gradient['b'] =np.sum(grad, axis=0).reshape(self.gradient['b'].shape)
...
class relu:
    ...
    def forward(self, X):
        ...
        forward_output = np.maximum(zero_array,X)
    ...
    def backward(self, X, grad):
        ...
        Heavis = np.zeros(X.shape)
        positivity = X > 0

        Heavis[positivity] = 1

        backward_output = np.multiply(grad,Heavis)

        backward_output = np.reshape(backward_output,grad.shape)
