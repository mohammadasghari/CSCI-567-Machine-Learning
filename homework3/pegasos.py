...
def objective_function(X, y, w, lamb):
    ...
    obj_value = (lamb/2.0)*(np.linalg.norm(w)**2) + np.sum(np.maximum(np.zeros([X.shape[0],1]), np.ones([X.shape[0],1]) - np.multiply(y,np.dot(X,w))))/X.shape[0]
...
def pegasos_train(Xtrain, ytrain, w, lamb, k, max_iterations):
    ...
    for iter in range(1, max_iterations + 1):
        ...
        A_t_plus = [] # index of sample satisfying yw^Tx<1


        for n in A_t:
            if ytrain[n]*np.dot(Xtrain[n,:],w) <1:
                A_t_plus.append(n)

        eta_t = 1.0/(lamb*iter)

        X_train_batch = Xtrain[tuple(A_t_plus),:]
        y_train_batch = ytrain[[A_t_plus]]

        y_train_batch = y_train_batch.reshape(len(y_train_batch), 1)

        w_mid = (1- eta_t*lamb)*w + (eta_t/k)*np.dot(np.transpose(X_train_batch),y_train_batch)

        w = min(1, 1.0/(np.linalg.norm(w_mid)*np.sqrt(lamb)))*w_mid
...
def pegasos_test(Xtest, ytest, w, t = 0.):
    ...
    y_pred_raw = np.dot(Xtest,w).reshape(Xtest.shape[0])

    y_pred = -1*np.ones(len(y_pred_raw))

    positivity = y_pred_raw >=t

    y_pred[positivity] =1

    num_correct_classified = np.sum(ytest == y_pred)

    test_acc = num_correct_classified/Xtest.shape[0]
